...
PHOTO_INSTRUCTION = '\nВажно, чтобы лицо было хорошо видно. ' \
                    '\n1) В ответ на это сообщение вы можете просто прислать мне файл с вашим фото.' \
                    '\n2) Если вы шлёте ссылку, она должна быть не на страничку с фото, а на сам файл ' \
                    '(т.е. должна иметь расширение типа .png, .jpg и т.п. в конце ссылки).' \
                    '\n3) Если у вас нет ссылки, можно загрузить фото, например, на vfl.ru. ' \
                    'Потом оттуда надо будет скопировать ПРЯМУЮ ссылку (последнее окошко).' \
                    '\n4) Ещё можно взять ссылку из соцсети: кликнуть правой кнопкой по фото на вашей страничке,' \
